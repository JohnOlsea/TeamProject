    ...
    <div className="app-container">
      <header className="am-header">
        <div className="am-header-content">
          ...
          <div>
            <h1 className="am-title">Print All Graduation Day Students</h1>
...
<tbody>
  {data
    .filter((row) => row.grant_option === "Graduation Day Pickup")
    .map((row, index) => (
        <tr key={index}>
        <td>{row.id}</td>
        <td style={{ textAlign: "left" }}>{row.name}</td>
...
export default AdminPrintGraduationDayStudents;